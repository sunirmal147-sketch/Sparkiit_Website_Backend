...
export const createBadge = async (req: Request, res: Response): Promise<void> => {
    try {
        const newItem = new Badge(req.body);
        await newItem.save();
...
export const updateBadge = async (req: Request, res: Response): Promise<void> => {
    try {
        const updatedItem = await Badge.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
...
export const deleteBadge = async (req: Request, res: Response): Promise<void> => {
    try {
        const deletedItem = await Badge.findByIdAndDelete(req.params.id);
